 ...
 */
export function analyzeAlignment(initiator, responder) {
    const conceptAlignment = alignConcepts(initiator.mentalModel, responder.mentalModel);
    const assumptionAlignment = alignAssumptions(initiator.mentalModel, responder.mentalModel);
    const goalAlignment = alignGoals(initiator.mentalModel, responder.mentalModel);
    const capabilityAlignment = alignCapabilities(initiator.capabilities, responder.capabilities);
    // Weighted overall score
    const score = conceptAlignment.score * 0.35 +
    ...
    const compatibilityFlags = determineCompatibility(score, divergences, initiator, responder);
    return {
        score,
 ...
 */
function alignCapabilities(a, b) {
 ...
 */
function generateResolutions(divergences) {
 ...
 */
function determineCompatibility(score, divergences, initiator, responder) {
 ...
 */
export function quickAlignmentScore(a, b) {
    // Just check task understanding similarity
    return calculateStringSimilarity(a.taskUnderstanding.toLowerCase(), b.taskUnderstanding.toLowerCase());
}
