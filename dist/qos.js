 ...
 */
export var TrafficClass;
(function (TrafficClass) {
    /** Background sync, non-urgent updates */
    TrafficClass[TrafficClass["BACKGROUND"] = 0] = "BACKGROUND";
    /** Standard context exchange */
    TrafficClass[TrafficClass["BEST_EFFORT"] = 1] = "BEST_EFFORT";
    /** Important but not critical */
    TrafficClass[TrafficClass["ASSURED"] = 2] = "ASSURED";
    /** Time-sensitive collaborative work */
    TrafficClass[TrafficClass["INTERACTIVE"] = 3] = "INTERACTIVE";
    /** Critical corrections, safety signals */
    TrafficClass[TrafficClass["CRITICAL"] = 4] = "CRITICAL";
    /** Emergency override - safety/shutdown */
    TrafficClass[TrafficClass["EMERGENCY"] = 5] = "EMERGENCY";
...
export class ContextClassifier {
    ...
    initializeDefaultRules() {
        ...
        this.rules.push({
            match: (ctx) => ctx.contextType === ContextType.EMERGENCY ||
...
export class PriorityQueue extends EventEmitter {
    ...
    initializeQueues() {
        ...
        for (const tc of Object.values(TrafficClass).filter(v => typeof v === 'number')) {
            this.queues.set(tc, []);
    ...
    dequeue() {
        ...
        const nonEmpty = Object.values(TrafficClass).filter(v => typeof v === 'number')
        ...
        switch (policy.exceedAction) {
            ...
            case 'remark':
                const newMarking = {
                    ...
                    dropPrecedence: Math.min(2, marking.dropPrecedence + 1),
                };
...
export class SLAMonitor extends EventEmitter {
     ...
     */
    recordPacket(tc, latency, dropped = false) {
    ...
    recordViolation(tc, metric, expected, actual) {
        const violation = {
            trafficClass: tc,
            metric,
            expected,
    ...
    getComplianceStatus() {
        ...
        for (const tc of Object.values(TrafficClass).filter(v => typeof v === 'number')) {
            const recentViolations = this.violations.filter(v => v.trafficClass === tc && now - v.timestamp < recentWindow);
            const metrics = this.metrics.get(tc);
            const latencies = metrics.latencies.map(l => l.value);
            status.set(tc, {
...
export class QoSManager extends EventEmitter {
     ...
     */
    submit(payload, contextType, source, destination, explicitMarking) {
        if (!this.enabled) {
            // Bypass QoS
    ...
    getStats() {
        ...
        const packetsProcessed = new Map();
        const packetsDropped = new Map();
        const averageLatency = new Map();
        const bandwidthUsage = new Map();
        for (const tc of Object.values(TrafficClass).filter(v => typeof v === 'number')) {
