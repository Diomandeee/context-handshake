...
// ============================================================================
export const DEFAULT_BANDWIDTH = {
    maxChunkSize: 4096,
    throttleMs: 50,
    priorityOrder: [
        'capabilities',
...
export class ContextChunker {
    maxChunkSize;
    constructor(maxChunkSize = DEFAULT_BANDWIDTH.maxChunkSize) {
        this.maxChunkSize = maxChunkSize;
    }
    /**
     * Split a context offer into streamable chunks
     */
    chunk(context, priorityOrder) {
        const chunks = [];
    ...
    splitDomain(domain, data, startSequence) {
        ...
        for (const [key, value] of entries) {
            ...
            if (currentSize + entrySize > this.maxChunkSize && currentSize > 0) {
                // Emit current batch
                chunks.push(this.createChunk(domain, chunkIndex, startSequence + chunkIndex, currentBatch, chunkIndex === 0 ? 'full' : 'delta', chunkIndex > 0 ? [`${domain}_${chunkIndex - 1}`] : undefined));
                chunkIndex++;
                currentBatch = {};
                currentSize = 0;
            }
    ...
    createChunk(domain, index, sequence, data, type, dependencies) {
        const serialized = JSON.stringify(data);
        return {
...
export class StreamingHandshake {
     ...
     */
    async initiateStream(initiatorId, responderId, context, onProgress) {
        ...
        let lastAlignment = { overall: 0, conceptMatch: 0, assumptionAlign: 0, goalOverlap: 0, capabilityComplement: 0 };
     ...
     */
    async respondToStream(sessionId, responderContext, onProgress) {
     ...
     */
    async completeHandshake(sessionId) {
        const session = this.sessions.get(sessionId);
        if (!session) {
            throw new Error(`Session ${sessionId} not found`);
        }
    ...
    // Private Helpers
    // --------------------------------------------------------------------------
    async calculateProgressiveAlignment(session, latestChunk) {
    ...
    shouldTerminate(session, snapshot) {
        const { earlyTermination } = session.bandwidth;
        if (!earlyTermination.enabled)
            return false;
        if (snapshot.chunksProcessed < earlyTermination.minChunks)
            return false;
    ...
    buildMergedModel(session) {
        ...
        return {
            sharedConcepts: merged['key_concepts'] || {},
            combinedCapabilities: merged['capabilities']?.items || [],
            reconciledAssumptions: merged['assumptions']?.items || [],
            taskUnderstanding: merged['task_understanding']?.content || '',
...
// Visualization
// ============================================================================
export function visualizeStreamProgress(session) {
    const lines = [];
    lines.push(`\n┌─────────────────────────────────────────────────────────────┐`);
...
// Demo
// ============================================================================
export async function demoStreamingHandshake() {
    ...
    const result = await engine.initiateStream('agent_alpha', 'agent_beta', largeContext, (snapshot) => {
