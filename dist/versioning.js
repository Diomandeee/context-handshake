...
export class ContextRepository {
    commits = new Map();
    branches = new Map();
    tags = new Map(); // Tag name → commit ID
    currentBranch = 'main';
    workingState;
    agentId;
    ...
    updateBranchHead(branchName, commitId) {
     ...
     */
    merge(sourceBranch, strategy = this.defaultStrategy, message) {
        ...
        const mergedUnderstanding = new Map();
        // Merge concepts
    ...
    mergeValue(type, key, ours, theirs, base, strategy) {
        // No conflict cases
        ...
        switch (strategy) {
            ...
            case 'confidence':
                if (type === 'concept') {
                    const oursConf = ours.confidence || 0;
                    const theirsConf = theirs.confidence || 0;
            ...
            case 'union':
                if (type === 'concept') {
                    autoResolution = this.unionConcepts(ours, theirs);
                    confidence = 0.7;
                }
                else if (type === 'belief') {
                    autoResolution = this.unionBeliefs(ours, theirs);
                    confidence = 0.7;
                }
                else {
                    // For understanding, take max
                    autoResolution = Math.max(ours, theirs);
                    confidence = 0.9;
                }
                break;
            case 'manual':
    ...
    isAncestor(potentialAncestor, descendant) {
        let current = this.commits.get(descendant);
        while (current) {
            if (current.id === potentialAncestor)
                return true;
    ...
    log(options = {}) {
        ...
        while (queue.length > 0 && (options.limit === undefined || commits.length < options.limit)) {
            ...
            if (options.author && commit.author !== options.author)
                continue;
            if (options.grep && !commit.message.toLowerCase().includes(options.grep.toLowerCase()))
                continue;
            commits.push(commit);
            queue.push(...commit.parentIds);
        }
    ...
    // Tags
    // --------------------------------------------------------------------------
    createTag(name, commitId, message) {
...
export class DistributedVersioning {
    ...
    processSyncMessage(message) {
        const result = { needsPull: [], needsPush: [], inSync: [] };
     ...
     */
    createPullRequest(remoteName, branch) {
...
// Usage Example
// ============================================================================
export function createCollaborationSession(agentA, agentB) {
