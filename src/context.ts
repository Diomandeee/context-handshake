 ...
 */
export function hashContext(context: AgentContext): string {
 ...
 */
function normalizeContext(context: AgentContext): object {
  return {
...
function normalizeMentalModel(model: MentalModel): object {
  return {
...
export function buildMentalModel(options: {
  ...
  const assumptions: Assumption[] = (options.assumptions || []).map((stmt, i) => ({
    id: `assumption_${i}`,
    statement: stmt,
    confidence: 0.8,
    basis: 'implicit',
  ...
  const goals: Goal[] = (options.goals || []).map((desc, i) => ({
    id: `goal_${i}`,
    description: desc,
    priority: i + 1,
    status: 'active',
 ...
 */
export function extractConcepts(text: string): Record<string, ConceptDefinition> {
  const concepts: Record<string, ConceptDefinition> = {};
 ...
 */
export function deserializeContext(data: string): AgentContext {
 ...
 */
export function compressContext(context: AgentContext): AgentContext {
  return {
    ...context,
    mentalModel: {
      ...context.mentalModel,
      ...
      keyConcepts: Object.fromEntries(
        Object.entries(context.mentalModel.keyConcepts)
 ...
 */
export function generateSessionId(participants: string[]): string {
