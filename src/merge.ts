...
export function mergeModels(
  ...
  const resolutions = new Map(
    alignment.proposedResolutions.map(r => [r.divergenceId, r])
  );

  // Merge each component
  const mergedConcepts = mergeConcepts(
    initiator.keyConcepts,
    responder.keyConcepts,
    resolutions
  );
  ...
  const mergedTaskUnderstanding = mergeTaskUnderstanding(
    initiator.taskUnderstanding,
    responder.taskUnderstanding,
    alignment.score
  );

  // Create merged mental model
  const sharedUnderstanding: MentalModel = {
...
function mergeConcepts(
  a: Record<string, ConceptDefinition>,
  b: Record<string, ConceptDefinition>,
  resolutions: Map<string, Resolution>
): Record<string, ConceptDefinition> {
  const merged: Record<string, ConceptDefinition> = {};
 ...
 */
function mergeConstraints(a: Constraint[], b: Constraint[]): Constraint[] {
  ...
  return merged.sort((x, y) => {
    if (x.type === 'hard' && y.type !== 'hard') return -1;
    if (x.type !== 'hard' && y.type === 'hard') return 1;
    return 0;
  });
}
 ...
 */
function mergeTaskUnderstanding(
  a: string,
  b: string,
 ...
 */
function synthesizeDefinitions(a: string, b: string): string {
...
function higherImportance(
  a: ConceptDefinition['importance'],
  b: ConceptDefinition['importance']
 ...
 */
export function updateMergedModel(
  current: MergedModel,
  update: Partial<MentalModel>,
  source: 'lead' | 'support'
): MergedModel {
