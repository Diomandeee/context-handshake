...
export enum StreamErrorCode {
  ...
  REFUSED_STREAM = 0x7,
  CANCEL = 0x8,
  CONTEXT_OVERFLOW = 0x9,
  ALIGNMENT_FAILED = 0xa,
  TIMEOUT = 0xb,
}
...
export class ConnectionPool extends EventEmitter {
  ...
  private agentConnections: Map<string, Set<string>> = new Map(); // agent -> connection ids
  ...
  async initialize(): Promise<void> {
    ...
    const warmupPromises = this.config.warmupAgents.map(agent =>
      this.warmConnection(agent).catch(err => {
  ...
  async acquire(
    ...
    if (!this.canCreateConnection(remoteAgent)) {
      ...
      throw new PoolExhaustedError(
        `Cannot create connection to ${remoteAgent}: pool limits reached`
      );
    }
  ...
  openStream(
    ...
    topic: string,
    priority: Partial<StreamPriority> = {}
  ): MultiplexedStream {
    if (connection.state !== ConnectionState.READY) {
      throw new ConnectionNotReadyError(
        `Connection ${connection.id} is ${connection.state}, cannot open stream`
      );
    }
    
    if (connection.streams.size >= this.config.maxStreamsPerConnection) {
      throw new StreamLimitError(
        `Connection ${connection.id} at max streams (${this.config.maxStreamsPerConnection})`
      );
    }
  ...
  closeStream(
    connection: PooledConnection,
    streamId: StreamId,
    errorCode: StreamErrorCode = StreamErrorCode.NO_ERROR
  ): void {
   ...
   * Get pool statistics
   */
  getStats(): PoolStats & { currentState: PoolState } {
  ...
  private makePoolKey(local: string, remote: string): PoolKey {
    return `${local}<->${remote}`;
  }
  ...
  private findReadyConnection(poolKey: PoolKey): PooledConnection | undefined {
    for (const connection of this.connections.values()) {
  ...
  private canCreateConnection(remoteAgent: string): boolean {
  ...
  private async warmConnection(remoteAgent: string): Promise<void> {
  ...
  private async waitForEmpty(timeout: number): Promise<void> {
  ...
  private calculateConnectionHealth(connection: PooledConnection): number {
    const metrics = connection.metrics;
    if (metrics.totalStreams === 0) return 1.0;
  ...
  private updateAcquisitionStats(time: number): void {
  ...
  private updateLatencyStats(connection: PooledConnection, latency: number): void {
    ...
    connection.metrics.p99StreamLatency = Math.max(
      connection.metrics.p99StreamLatency,
      latency
    );
  }
  
  private isRecoverableError(code: StreamErrorCode): boolean {
    return [
...
export class PriorityScheduler {
  private root: PriorityNode;
  private nodes: Map<StreamId, PriorityNode> = new Map();
  
  constructor() {
    this.root = {
      streamId: 'root' as StreamId,
      weight: 16,
      children: [],
      parent: null,
    };
   ...
   */
  schedule(activeStreams: Set<StreamId>): Array<{ streamId: StreamId; allocation: number }> {
    const allocations: Array<{ streamId: StreamId; allocation: number }> = [];
  ...
  private scheduleNode(
    ...
    result: Array<{ streamId: StreamId; allocation: number }>
  ): void {
    // Filter to active children
    const activeChildren = node.children.filter(c => 
      activeStreams.has(c.streamId) || this.hasActiveDescendant(c, activeStreams)
    );
  ...
  private hasActiveDescendant(node: PriorityNode, activeStreams: Set<StreamId>): boolean {
    for (const child of node.children) {
      if (activeStreams.has(child.streamId)) return true;
      if (this.hasActiveDescendant(child, activeStreams)) return true;
    }
    return false;
  }
}
...
export async function quickStream(
  ...
  remoteAgent: string,
  topic: string
): Promise<{ connection: PooledConnection; stream: MultiplexedStream }> {
  const pool = getPool();
 ...
 */
export function visualizePool(pool: ConnectionPool): string {
